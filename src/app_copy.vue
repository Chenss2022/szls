<template>
  <div id="canvasFor" class="canvasFor" style="width: 100%;height: 100%;user-select: none;">
    <dv-border-box11 title="数字孪生" :title-width="400" :animate="true" style="position: absolute;pointer-events: none;">
      <dv-border-box13 style="width:23vw;height: 9vh;position: absolute;top: 5vh;left: 2vw;color: white;" :backgroundColor="'#cccccc50'">
      ...
      <dv-border-box8 v-if="!Data.winOpenClose" style="position: absolute;width: 23vw;height: 25vh;top: 15vh;left: 2vw;color: white;" :dur="5" :backgroundColor="'#cccccc50'">
      ...
      <div v-if="!Data.winOpenClose" style="position: absolute;width:23vw;height:19vh;top: 45vh;left: 2vw;color:white;">
      ...
      <dv-border-box8 v-if="!Data.winOpenClose" style="position: absolute;width: 23vw;height: 25vh;;bottom: 5vh;left:2vw;color: white;" :dur="5" :backgroundColor="'#cccccc50'">
       <div style="display: flex;flex-flow: column;justify-content: space-around;height: 100%;">
        <div>库存分析</div>
        <div>
      ...
      <dv-border-box13 style="width:23vw;height: 6vh;position: absolute;top: 5vh;right: 2vw;color: white;" :backgroundColor="'#cccccc50'">
        <div style="margin-top: 2vh;">
          <div>{{ Data.DayJs }}</div>
        </div>
      </dv-border-box13>
      <dv-border-box8 v-if="!Data.winOpenClose" :reverse="true" style="width:23vw;height: 26vh;position: absolute;top: 13vh;right: 2vw;color: white;" :backgroundColor="'#cccccc50'">
      ...
      <dv-border-box1 v-if="!Data.winOpenClose" ref="borderRef" style="position: absolute;width:23vw;height: 29vh;top: 45vh;right: 2vw;color: white;" :backgroundColor="'#cccccc50'">
        <div>出入库情况</div>
        <dv-decoration5 :dur="2" style="margin: 0 auto;width:300px;height:40px;margin-top: -2.5vh;" />
        <div>
          出库
          <div>粮食品种：{{ Data.outStockInfo[0]?.echoMap?.grainType }}</div>
          <div>最后出库时间：{{ Data.outStockInfo[0]?.lastOutStockTime }}</div>
          <div>累计出库量：{{ Data.outStockInfo[0]?.totalOutStockCount }} 出库车次：{{ Data.outStockInfo[0]?.outStockTimes }}</div>
        </div>
        <div>
          入库
          <div>粮食品种：{{ Data.inStockInfo[0]?.echoMap?.grainType }}</div>
          <div>最后入库时间：{{ Data.inStockInfo[0]?.lastInStockTime }}</div>
          <div>累计入库量：{{ Data.inStockInfo[0]?.totalInStockCount }} 入库车次：{{ Data.inStockInfo[0]?.inStockTimes }}</div>
        </div>
      </dv-border-box1>
      <div v-if="!Data.winOpenClose" style="width:23vw;height: 23vh;position: absolute;bottom: 3vh;right: 2vw;color: white;">
        <div>告警分析</div>
        <div style="display: flex;flex-flow: row;">
      ...
      <dv-button :bg="true" @click="reserves" style="pointer-events: auto;position: absolute;bottom: 3vh;right: 53vw;">复位</dv-button>
      <dv-button :bg="true" @click="showFalse" style="pointer-events: auto;position: absolute;bottom: 3vh;right: 45vw;" :backgroundColor="'#cccccc50'">{{Data.showFalseBoo == true?'标签显隐':'标签显示'}}</dv-button>
    </dv-border-box11>
    <!-- 详情显示库存容量 -->
    <dv-border-box8 v-if="Data.winOpenClose" style="position: absolute;width: 23vw;height: 25vh;;top: 15vh;left:2vw;color: white;" :dur="5" :backgroundColor="'#cccccc50'">
      <div style="display: flex;flex-flow: column;justify-content: space-around;height: 100%;">
    ...
    <!-- 详情显示出入库信息 -->
    <dv-border-box1 v-if="Data.winOpenClose" ref="borderRef" style="position: absolute;width:23vw;height: 50vh;top: 45vh;left: 2vw;color: white;" :backgroundColor="'#cccccc50'">
      <div>出入库情况</div>
      <dv-decoration5 :dur="2" style="margin: 0 auto;width:300px;height:40px;margin-top: -2.5vh;" />
      <div style="line-height: 4vh;">
        出库
        <div>粮食品种：{{ Data.outStockInfoView[0]?.echoMap?.grainType }}</div>
        <div>最后出库时间：{{ Data.outStockInfoView[0]?.lastOutStockTime }}</div>
        <div>累计出库量：{{ Data.outStockInfoView[0]?.totalOutStockCount }} 出库车次：{{ Data.outStockInfoView[0]?.outStockTimes }}</div>
      </div>
    ...
    <!-- 详情显示货位卡 -->
    <dv-border-box1 v-if="Data.winOpenClose" ref="borderRef" style="position: absolute;width:23vw;height: 83vh;top: 13vh;right: 2vw;color: white;" :backgroundColor="'#cccccc50'">
      <div>货位卡信息</div>
    ...
    <!-- 弹窗 -->
    <dv-border-box12 v-show="Data.winOpenClose ||Data.cameraDataFoo" style="position: absolute;left: 32vw;top: 30vh;width: 36vw;height: 20vw;" :backgroundColor="'#cccccc50'">
      <!-- 粮情信息 -->
