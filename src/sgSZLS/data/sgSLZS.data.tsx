...
let api = 'http://192.168.3.98:18760';
...
export function axiosItem(data){
    // 货位卡接口
    axios.post(api+'/api/digitaltwin/digitalTwinBarnInfo/electronicStorageCard',{hwdm:data},{headers:{...headersData,'Content-Type':'application/json'}}).then((res)=>{
        handleItem.electronicStorageCard = res.data.data
    })
    // 库存分析
    axios.post(api+'/api/digitaltwin/digitalTwinBarnInfo/inventoryAnalysis',{hwdm:data},{headers:{...headersData,'Content-Type':'application/json'}}).then((res)=>{
        handleItem.inventoryAnalysisView = res.data
    })
    // 入库情况
    axios.post(api+'/api/digitaltwin/digitalTwinBarnInfo/inStockInfo',{hwdm:data},{headers:{...headersData,'Content-Type':'application/json'}}).then((res)=>{
        handleItem.inStockInfoView = res.data.data
    })
    // 出库情况
    axios.post(api+'/api/digitaltwin/digitalTwinBarnInfo/outStockInfo',{hwdm:data},{headers:{...headersData,'Content-Type':'application/json'}}).then((res)=>{
        handleItem.outStockInfoView = res.data.data
    })
    // 粮情温度
    axios.post('/api/digitaltwin/digitalTwinBarnInfo/temperatureAndHumidity',{hwdm:data},{headers:{...headersData,'Content-Type':'application/json'}}).then((res)=>{
